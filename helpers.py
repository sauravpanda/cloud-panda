...
def reply_to_comment(payload, access_token):
    ...
    comment_title = payload['issue']['title']
    ...
    resp = generate_llm_resp(f"Help a developer with this problem: title :- {comment_title}\n body:- {comment_body}")
    data = {
        'body': f'Noted this issue. Here is what I know about this issue. \n {resp} \n If this answers your query, please close the request or wait someone to respond.'
    }

    if "Generated by Cloud Panda" not in comment_body:
        print(reply_url)
...
def reply_to_r2r_comments(payload, access_token):
    repo_owner = payload['repository']['owner']['login']
    repo_name = payload['repository']['name']
    issue_number = payload['issue']['number']
    comment_id = payload['comment']['id']
...
def review_code(payload, access_token):
